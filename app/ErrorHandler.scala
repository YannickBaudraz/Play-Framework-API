...
@Singleton class ErrorHandler @Inject() (
  ...
  override def onClientError(request: RequestHeader, statusCode: Int, message: String): Future[Result] = {
    ...
    Future.successful {
      statusCode match {
        case BAD_REQUEST =>
          BadRequest(JsonError("Bad Request", message))
        case FORBIDDEN =>
          Forbidden(JsonError("Forbidden", message))
        case NOT_FOUND =>
          NotFound(JsonError("Not Found", message))
  ...
  override protected def onDevServerError(request: RequestHeader, exception: UsefulException): Future[Result] = {
    ...
    exception.getCause match {
      case e: NoSuchElementException =>
        Future.successful(NotFound(JsonError("Not Found", e.getMessage)))
      case _ =>
        Future.successful(
          InternalServerError(JsonError("Internal Server Error", exception.getMessage))
        )
    }
  }
